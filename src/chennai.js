...
const Chennai = () => {
  ...
  useEffect(() => {
    // Fetch data from the PHP endpoint using Axios
    axios.get('http://localhost:80/TICKETAPI/chennai.php')
      .then(response => {
      ...
      <div className='row'>
        {events.map(event => (
          <div className="col-4 mb-4" key={event.id}>
            <div className="card">
              <img src={`data:image/jpeg;base64,${event.image}`} className="card-img-top img-fluid" alt={event.title} style={{ height: '300px', objectFit: 'cover' }} />
